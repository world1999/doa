    ...
    def steering_vec(
        ...
        if array_form.startswith("ULA"):
            ...
            # If calculation is applied through method (array mismatches are not known).
                ...
                mis_distance, mis_geometry_noise,uniform_bias = 0, 0, 0#额外增加了初始化uniform_bias，避免未赋值
            ...
            return (
                np.exp(
                    ...
                    * 1j
                    * np.pi
                    * f_sv[self.params.signal_type]#单频率的和频率无关，如果假设为半波长间距的阵列
                    * (uniform_bias + mis_distance + self.dist[self.params.signal_type])#d/lambda=1/2
                    * self.array
                    * np.sin(theta)
                )
                + mis_geometry_noise
